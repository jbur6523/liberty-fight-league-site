...
function confirmationPayload(response = "awaiting", gym = "North Bay Jiu-Jitsu") {
  return {
    ...
    match: { weightLbs: 155, weightOption: previewEvent.weightOptions[1], boutType: "gi", active: true },
...
async function mockApi(request, response, url) {
  ...
  if (url.pathname === "/api/superfight-status") {
    return json(response, 200, {
      ...
      event: { name: previewEvent.name, startsAt: previewEvent.startsAt, venue: previewEvent.venue },
      match: { weightLbs: 155, weightOption: previewEvent.weightOptions[1], boutType: "gi", confirmation: { fighterA: "accepted", fighterB: "awaiting", summary: "fighter_a_accepted" } },
      status: "matched",
    });
  }
